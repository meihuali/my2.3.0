...
public class DimensionUtils {
    ...
    public static DisplayMetrics getDisplayMetrics() {
        return App.getsInstance().getResources().getDisplayMetrics();
    }

    public static int getStatuBarHeight() {
        int result = (int) dp2px(24);
        int resourceId = App.getsInstance().getResources().getIdentifier("status_bar_height", "dimen", "android");
        if (resourceId > 0) {
            result = App.getsInstance().getResources().getDimensionPixelSize(resourceId);
        }
        return result;
    }
    ...
    public static int getStatusBarHeight() {
        // 获得状态栏高度
        Resources resources = App.getsInstance().getResources();
